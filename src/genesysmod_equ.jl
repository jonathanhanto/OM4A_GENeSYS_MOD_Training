...
#=       if Params.TradeRoute[r,rr,f,𝓨[i]] == 0
 ...
 ############### Total Discounted Costs #############
  ...
  for i ∈ eachindex(𝓨) for f ∈ 𝓕 for r ∈ 𝓡
    ...
    if Switch.switch_dispatch isa NoDispatch
      if 𝓨[i]> 2035 && Params.SpecifiedAnnualDemand[r,f,𝓨[i]]>0 && Params.SpecifiedAnnualDemand[r,f,𝓨[i-1]]>0
        @constraint(model,
  ...
  if Switch.switch_peaking_capacity == 1
    ...
    for y ∈ 𝓨 for r ∈ 𝓡
      ...
      if y >Switch.set_peaking_startyear
        ...
        + (Switch.switch_peaking_with_storages == 1 ? sum(Vars.TotalCapacityAnnual[y,t,r] for t ∈ setdiff(𝓣,Params.Tags.TagTechnologyToSubsets["StorageDummies"]) if (sum(Params.OutputActivityRatio[r,t,"Power",m,y] for m ∈ Maps.Tech_MO[t]; init=0.0) != 0)) : 0)
