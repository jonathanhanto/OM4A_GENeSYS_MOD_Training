...
function genesysmod_dec(model,Sets, Params,Switch,Maps)
    ...
    OperatingCost = @variable(model, OperatingCost[𝓨,𝓣,𝓡], container=JuMP.Containers.DenseAxisArray)
    DiscountedOperatingCost = @variable(model, DiscountedOperatingCost[𝓨,𝓣,𝓡], container=JuMP.Containers.DenseAxisArray)
    AnnualVariableOperatingCost = @variable(model, AnnualVariableOperatingCost[𝓨,𝓣,𝓡], container=JuMP.Containers.DenseAxisArray)
    AnnualFixedOperatingCost = @variable(model, AnnualFixedOperatingCost[𝓨,𝓣,𝓡], container=JuMP.Containers.DenseAxisArray)
    VariableOperatingCost = @variable(model, VariableOperatingCost[𝓨,𝓛,𝓣,𝓡], container=JuMP.Containers.DenseAxisArray)
    TotalDiscountedCost = @variable(model, TotalDiscountedCost[𝓨,𝓡], container=JuMP.Containers.DenseAxisArray)
    TotalDiscountedCostByTechnology = @variable(model, TotalDiscountedCostByTechnology[𝓨,𝓣,𝓡], container=JuMP.Containers.DenseAxisArray)
    ModelPeriodCostByRegion = @variable(model, ModelPeriodCostByRegion[𝓡], container=JuMP.Containers.DenseAxisArray)
