...
function genesysmod_results(model,Sets, Params, Vars, VarPar, Switch, Settings, elapsed, extr_str)
    ...
    df_tmp = DataFrame(Dict(:Region => "General", :Region2 => "General",:Type => "Transmission Expansion Costs in MEUR/GW/km",:Year => Sets.Year,:Value => Params.TradeCapacityGrowthCosts[Sets.Region_full[1],Sets.Region_full[r2],"Power"]))
    select!(df_tmp,colnames)
    append!(output_trade_capacity, df_tmp)
